...
# Summary:      Up to date version with debugging work to validate optimizer, energy version. Some development & patches are backported from electric version. Fixed energyLimit
# Changelog:
# - Fixed energyLimit (for cooling at least)
# - Add compatibility for real PG&E rate pricing data - Originally run in this version!
...
# 'r' = real-time
# 'd' = day-ahead
# 'E-TOU-C_Summer, E-TOU-C_Winter, E-1'
wholesaleType = 'r'
...
try: #For dealing with various errors if the sectionName is not found in .ini file
    ...
    #Changed to handle fancier pricing systems by season and location - 5/22/22
    try: 
        PRICING_MULTIPLIER = float(cfp.get(sectionName, 'PRICING_MULTIPLIER'))
        PRICING_OFFSET = float(cfp.get(sectionName, 'PRICING_OFFSET'))
    except (NameError, configparser.NoSectionError):
    ...
    c4 = 0
    PRICING_MULTIPLIER = 15.0 
    PRICING_OFFSET = 0.005
...
startdat = (block-1)*12+1 #added +1 on 10/1/22 to try to mitigate timestep delay in EP
...
energyLimit = matrix(0.069, (n,1)) # .4, 0.25 before. Got this using max of cooling energy [j] out of EP = 0.069.
# enforce |E_used[n]| <= E_max
heatlimiteq = (cool_negative * x <= energyLimit)
coollimiteq = (heat_positive * x <= energyLimit)
