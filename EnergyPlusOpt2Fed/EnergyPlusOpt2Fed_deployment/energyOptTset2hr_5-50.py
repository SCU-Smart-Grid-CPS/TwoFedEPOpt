...
# - NOT compatible with ANY Older Versions! Legacy no longer supported! - this version keeps old code; v5.51 discards for brevity.
...
S = matrix(0.0, (n,1))
S[0,0] = timestep*(c1*(temp_outdoor[0]-temp_indoor_initial)+c3*q_solar[0]+c4*q_sol_diffuse)+temp_indoor_initial
...
i=1
while i<n:
    S[i,0] = timestep*(c1*(temp_outdoor[i]-S[i-1,0])+c3*q_solar[i]+c4*q_sol_diffuse)+S[i-1,0]
    i+=1
...
temp_indoor[0,0] = timestep*(c1*(temp_outdoor[0,0]-temp_indoor[0,0])+c2*energy[0,0]+c3*q_solar[0,0]+c4*q_sol_diffuse[0,0]) + temp_indoor_initial
p = 1
while p<n:
    temp_indoor[p,0] = timestep*(c1*(temp_outdoor[p,0]-temp_indoor[p,0])+c2*energy[p,0]+c3*q_solar[p,0]+c4*q_sol_diffuse[0,0])+temp_indoor[p-1,0]
    p = p+1
