...
# Summary:      Up to date version with debugging work to validate optimizer, electricity version. Development was ported from the 5.59 series
# Changelog:
# - Added debugging noHVACtest and energytest
...
# - Merged 2 codes, keeping the best characteristics of each
...
if HRO:
    ...
    print()
    print('=========== energyOptTset2hr.py V5.64 BETA ===========')
...
if 'E' in wholesaleType:
    if 'E-1' in wholesaleType: pfile = 'E-1_5min.csv'
    elif 'E-TOU-C_Summer' in wholesaleType: pfile = 'E-TOU-C_5min_Summer.csv'
    elif 'E-TOU-C_Winter' in wholesaleType: pfile = 'E-TOU-C_5min_Winter.csv'
    elif 'E-TD-Z' in wholesaleType: pfile = 'E-TD-Z_5min.csv'
    else: #error
        pfile = 'Zero.csv'
...
# Using Demand-based CAISO pricing
    ...
    elif 'r' in wholesaleType.lower(): pfile = "WholesaleRealTime_" + date_range + ".csv"
...
if priceAvg:
    ...
    v1 = np.ones(n)
    cc = matrix(v1)*wholesaleAvg*PRICING_MULTIPLIER/1000+PRICING_OFFSET
...
E=variable(n)    # E is electricity usage that we are predicting (NOT Same as E_used on PJs thesis, page 26)

#Coefficient of Performance COP = COP_actual * Unknown_constant_coefficient
COP = matrix(0.0,  (n,1))
...
while k<n:
    ...
    while j<2*n-1:
        D[j,k] = D[j-2,k]*-timestep*c1+ D[j-2,k]
        D[j+1,k] = D[j-1,k]*-timestep*c1+ D[j-1,k]
        j+=2
    k=k+1

# Set signs on heat and cool energy ----------------------
# energy is positive for heating & negative for cooling
...
heatineq = (heat_positive*E<=d)
#           negative * positive <= [0]
coolineq = (cool_negative*E<=d)
#           positive * negative <= [0]

# Max heating and cooling capacity of HVAC system ------------
elecLimit = matrix(1000.0, (n,1)) # [W] I think?
# enforce |E_used[n]| <= E_max # Added abs() 2021-11-09 to fix it allowing infinite energy.
heatlimiteq = (abs(heat_positive * E) <= elecLimit)
coollimiteq = (abs(cool_negative * E) <= elecLimit)

# S matrix of predicted indoor temp without HVAC to make b matrix simpler -----------------
S = matrix(0.0, (n,1))
...
# b matrix ----------------------------------------------------
# b matrix is constant term in constaint equations, containing temperature discrepancy T_req - T_nat
b = matrix(0.0, (n*2,1))
# Initialize counter 
k = 0
...
if 'occupancy' in MODE:
    ...
    if MODE == 'occupancy': #For speed, putting this one first because it is the most common.
        ...
        if occupancy_status[0] == 1:
            ...
            # Do for the number of timesteps where occupancy is known truth
            while k < n_occ:
                # If occupied initially, use fixed setpoint for the first occupancy frame
...
# Adaptive 90% Averaged over 2 hours - test mode
    ...
    while k<n:
        spCool[k,0] = adaptCoolAvg
        spHeat[k,0] = adaptHeatAvg
        b[2*k,0]=adaptCoolAvg-S[k,0]
        b[2*k+1,0]=-adaptHeatAvg+S[k,0]
        k=k+1
...
k=0
if HRO_DEBUG: print('COP = ')
while k<n:
    COP[k,0] = abs(1 / (1- temp_outdoor[k,0] / sp[k,0]))
    #Human readable equation: TdiffCOP = (T_req - T_nat) / (COP * gamma)
    TdiffCOP[k,0] = b[k,0] / (COP[k,0] * gamma)
    if HRO_DEBUG: print(COP[k,0])
    k+=1
    

# Solve for energy at each timestep. Constraint D*E^n_used <= b, D = D, E^n_used = x
...
# S[n] is essentially temperature naturally occurring indoors if no HVAC, so b = T_req - T_nat
ineq = (D*E <= TdiffCOP)
...
if HRO_DEBUG:
    print('Before opt \nS = \n')
    print(S)
    print('\nTdiffCOP = (T_req - T_nat) / (COP * gamma) = \n')
    print(TdiffCOP)
    print('\nMatrix D = \n')
    print(D)
...
if 'h' in heatorcool: # PJ eq 2.8; T^(n)_indoor >= T^(n)_comfort,lower
    # Minimize price cc, electric E, with constraint ineq
    lp2 = op(dot(cc,E),ineq)
    ...
    lp2 = op(dot(-cc,E),ineq) #maybe because x is negative and price is positive?
...
electric = E.value 
electric = matrix(0.0, (n,1))

#For preset electricity testing
if MODE == 'electrictest': 
	#Jan 1, 6am
	#electric = matrix([0.555472075, 0.5441195959, 0.8251619103, 0.5680418427, 0.4328863549, 0.7550661865, 0.4663252374, 0.7081681671, 0.5305115967, 0.3080624774, 0.7729041968, 0.6151092013, 0.7972589359, 0.5283601257, 0.7599873747, 0.5120342401, 0.736130555, 0.4923572159, 0.71694203, 0.4722338945, 0.6973660948, 0.04613704547, 0.3618579578, 0.1645973253]) * 0.67;
	#Jan 1, 10am
	#electric = matrix([0.09121830997, 0.08858966966, 0.08618242959, 0.08380415656, 0.08146854992, 0.07914119599, 0.07681455049, 0.07470792546, 0.07267161682, 0.05655640989, 0.05542146523, 0.05428955333, 0.05314928897, 0.05312851111, 0.05223468544, 0.05135956088, 0.05046815455, 0.04959258648, 0.04874667085, 0.04795487004, 0.04717626723, 0.04639286578, 0.04561198501, 0.04482814534]) * 5.5;
	#Jan 2, 18:00
	#electric = matrix([0.05520429313, 0.05538052028, 0.05555830135, 0.05573851872, 0.05592152118, 0.05610777112, 0.05629762551, 0.05649135989, 0.05668917642, 0.05689121117, 0.05709754161, 0.05730819427, 0.05752315216, 0.05755812405, 0.05759906211, 0.05764425567, 0.05769326317, 0.05774597029, 0.05780225201, 0.05786196266, 0.0579249247, 0.05799091936, 0.05805967445, 0.0581308408, 0.05820393366])* 8.3;
	#Aug 1, hr 20
	#electric = matrix([0.555472075, 0.5441195959, 0.8251619103, 0.5680418427, 0.4328863549, 0.7550661865, 0.4663252374, 0.7081681671, 0.5305115967, 0.3080624774, 0.7729041968, 0.6151092013, 0.7972589359, 0.5283601257, 0.7599873747, 0.5120342401, 0.736130555, 0.4923572159, 0.71694203, 0.4722338945, 0.6973660948, 0.04613704547, 0.3618579578, 0.1645973253]) * -0.033;
	# Aug 1, Hr 11
	#electric = matrix([0.04002503583, 0.04014967304, 0.04101738416, 0.04185218464, 0.04271479738, 0.04358704646, 0.0444693617, 0.04533826292, 0.04622678689, 0.04713516196, 0.04806939897, 0.04903981636, 0.0500348815, 0.2579128691, 0.2543737514, 0.2530070042, 0.252669378, 0.2509320569, 0.2250074834, 0.2041144509, 0.190069227, 0.179677007, 0.1726100605, 0.1655539252]) * -0.5;
	# Aug 1, Hr 14
	electric = matrix([0.0485025127, 0.05372523123, 0.05903277688, 0.0643728894, 0.06972699849, 0.07507814259, 0.08041380131, 0.08554385909, 0.09058678983, 0.09555919202, 0.1004573573, 0.1052787466, 0.1100218946, 0.1149173088, 0.1197437616, 0.1245005252, 0.1291870577, 0.1338029559, 0.1383482038, 0.1425751357, 0.1466939817, 0.1506373978, 0.1545810132, 0.1584729039, 0.1622442348]) * -0.16;
...
if MODE == 'energytest': 
	...
	#energy = matrix([0.555472075, 0.5441195959, 0.8251619103, 0.5680418427, 0.4328863549, 0.7550661865, 0.4663252374, 0.7081681671, 0.5305115967, 0.3080624774, 0.7729041968, 0.6151092013, 0.7972589359, 0.5283601257, 0.7599873747, 0.5120342401, 0.736130555, 0.4923572159, 0.71694203, 0.4722338945, 0.6973660948, 0.04613704547, 0.3618579578, 0.1645973253]) * -0.09;
	# Aug 1, Hr 11
	#energy = matrix([0.04002503583, 0.04014967304, 0.04101738416, 0.04185218464, 0.04271479738, 0.04358704646, 0.0444693617, 0.04533826292, 0.04622678689, 0.04713516196, 0.04806939897, 0.04903981636, 0.0500348815, 0.2579128691, 0.2543737514, 0.2530070042, 0.252669378, 0.2509320569, 0.2250074834, 0.2041144509, 0.190069227, 0.179677007, 0.1726100605, 0.1655539252]) * -0.9;
	# Aug 1, Hr 14
	energy = matrix([0.0485025127, 0.05372523123, 0.05903277688, 0.0643728894, 0.06972699849, 0.07507814259, 0.08041380131, 0.08554385909, 0.09058678983, 0.09555919202, 0.1004573573, 0.1052787466, 0.1100218946, 0.1149173088, 0.1197437616, 0.1245005252, 0.1291870577, 0.1338029559, 0.1383482038, 0.1425751357, 0.1466939817, 0.1506373978, 0.1545810132, 0.1584729039, 0.1622442348]) * -0.4

print("Temps before")
print(temp_indoor_initial)
print(temp_indoor[0,0])
